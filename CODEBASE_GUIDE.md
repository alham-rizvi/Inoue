...
### 27. Browser extension is a thin API client

The shared Chrome/Firefox WebExtension lives in `extension/`. It reads the
active HTTP(S) tab and calls the existing API with `modules: ["fast"]`; it
does not duplicate fingerprinting logic or inspect page contents itself. API
URL and API key settings are stored in extension-local storage, and release
archives are generated by `scripts/build_extension.py`.

### 28. API scans reject private targets by default

The FastAPI layer validates target schemes, credentials, DNS addresses, and
redirect hops before scanning. Public-target mode is the default to prevent
the API from becoming an SSRF proxy; isolated lab users can explicitly set
`INOUE_ALLOW_PRIVATE_TARGETS=true`. CLI scans retain their existing behavior
